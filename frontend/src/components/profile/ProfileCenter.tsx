...
import MinerUVersionManager from "./MinerUVersionManager";
                      ...
                      <label className="space-y-1.5">
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            ...
                            if (llmProvider === "custom") setLlmModels([]);
                          }}
                          onBlur={() => {
                            if (llmProvider !== "custom") loadLlmModels(llmProvider, llmBaseUrl, llmApiKey, { quiet: true });
                          }}
                    ...
                    <div className={`rounded-xl border p-4 ${
                      ...
                      <div className="mb-3 flex items-center justify-between gap-2">
                        ...
                        <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${
                          labRetrievalAvailable ? "bg-stone-100 text-stone-700" : "bg-red-100 text-red-700"
                        }`}>
                          {labRetrievalAvailable ? "检索正常" : "检索异常"}
                        </span>
                      </div>
                      <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-semibold">
                        <div className={`rounded-lg border px-3 py-2 ${
                          ...
                          向量检索：{labRetrievalAvailable ? "正常" : "异常"}
                        </div>
                        <div className={`rounded-lg border px-3 py-2 ${
                          labOptimizationAvailable
                            ? "border-stone-200 bg-white text-stone-700"
                          ...
                          优化训练：{labOptimizationAvailable ? "正常" : "异常"}
                        </div>
                      </div>
                      <button
                        type="button"
                    ...
                    <section className="rounded-xl border border-slate-200 bg-white p-4">
                        <div className="mb-4 grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-xs leading-relaxed text-slate-500">
                              控制知识库 PDF 确认处理时优先使用实验室本地 MinerU，还是 MinerU 官方线上 API。API Key 只会加密保存，不会在前端回显。
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                          <label className="block text-sm font-semibold text-slate-700">
                            处理策略
                            <select
                              value={mineruProcessingConfig.provider_mode}
                              onChange={(event) => setMineruProcessingConfig((prev) => {
                                const next = {
                                  ...prev,
                                  provider_mode: event.target.value as MinerUProcessingConfigInfo["provider_mode"],
                                };
                            ...
                            >
                              <option value="lab_only">只使用实验室 MinerU</option>
                              <option value="cloud_only">只使用线上 MinerU API</option>
                              <option value="cloud_first">线上优先，失败后实验室</option>
                              <option value="lab_first">实验室优先，失败后线上</option>
                            </select>
                          </label>
                          <label className="block text-sm font-semibold text-slate-700">
                            MinerU API 地址
                          ...
                          <label className="block text-sm font-semibold text-slate-700 md:col-span-2">
                            ...
                            <span className="mt-2 block text-xs leading-relaxed text-slate-400">
                              {mineruProcessingConfig.api_key_configured
                                ...
                                : "未配置 API Key，选择线上 MinerU 前需要先填写。"}
                            </span>
                          </label>
                        </div>
                      ...
                      <section className="rounded-xl border border-slate-200 bg-white p-4">
                        <div className="mb-4">
                          <div className="flex flex-wrap items-center gap-2 text-base font-bold text-slate-900">
                            <span>MinerU 版本管理</span>
                          </div>
                          <p className="mt-1 text-xs leading-relaxed text-slate-500">
                            从 GitHub（国内镜像优先）下载指定 MinerU 版本，自动创建独立 conda 环境并冒烟测试通过后手动启用，支持回退，仅保留最近 3 个版本。有解析任务运行时禁止切换。
                          </p>
                        </div>
                        <MinerUVersionManager />
                      </section>
                  </div>
                )}
                    ...
                    <section className="rounded-xl border border-slate-200 bg-white p-4">
                        <div className="mb-4 grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-xs leading-relaxed text-slate-500">
                              控制文献检索的 embedding 与 rerank 模型使用实验室本地服务，还是硅基流动云端 API。API Key 只会加密保存，不会在前端回显。
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                          <label className="block text-sm font-semibold text-slate-700">
                            检索后端
                            <select
                              value={retrievalModelsConfig.provider_mode}
                              onChange={(event) => setRetrievalModelsConfig((prev) => {
                                const next = {
                                  ...prev,
                                  provider_mode: event.target.value as RetrievalModelsConfigInfo["provider_mode"],
                                };
                                void saveRetrievalModelsConfig(next);
                                return next;
                              })}
                              className="mt-2 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-50"
                            >
                              <option value="lab_only">实验室本地模型</option>
                              <option value="cloud_only">硅基流动云端 API</option>
                            </select>
                          </label>
                          <label className="block text-sm font-semibold text-slate-700">
                            API 地址
...
function mineruProcessingModeLabel(mode: MinerUProcessingConfigInfo["provider_mode"]) {
  if (mode === "cloud_only") return "只用线上";
  if (mode === "cloud_first") return "线上优先";
  if (mode === "lab_first") return "实验室优先";
  return "只用实验室";
}

function retrievalModeLabel(mode: RetrievalModelsConfigInfo["provider_mode"]) {
  if (mode === "cloud_only") return "云端 API";
  return "实验室模型";
}
