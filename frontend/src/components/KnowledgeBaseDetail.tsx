...
import { getLiteratureImageUrl, loadLiteratureImageUrl, loadLiteraturePdfUrl, type LiteratureFileInfo, type LiteratureFilePreview } from "../services/literature";
import RawDocumentViewer from "./RawDocumentViewer";
import { canUseDesktopCloudTasks } from "../desktop/services/cloudTasks";
...
export default function KnowledgeBaseDetail({
  ...
  // 桌面端云直链无法携带凭证，原始文档改为异步获取（Web 模式下会同步返回直链）
  ...
  useEffect(() => {
    ...
    setPdfUrl("");
    if (!showRawPdf || !currentFile) return () => {};

    void loadLiteraturePdfUrl(folderName, currentFile.name)
      .then(loaded => {
        if (disposed) {
          loaded.revoke?.();
          return;
  ...
  }, [currentFile?.name, folderName, showRawPdf]);

  const markdownComponents = useMemo<Components>(() => {
    const imageClassName = "mx-auto my-3 max-w-full rounded-lg border border-[#eadccf] bg-white shadow-sm";
    const resolveImageRel = (src?: string) => {
      if (!src) return null;
    ...
    return {
      img: ({ src, alt }) => {
        ...
        const rel = currentFile ? resolveImageRel(rawSrc) : null;
        if (rel && canUseDesktopCloudTasks()) {
          // Tauri 模式下直链带不上会话凭证，插图改经桌面云代理取 blob
          return <DesktopLiteratureImage folder={folderName} filename={currentFile!.name} image={rel} alt={alt || ""} className={imageClassName} />;
        }
        const resolved = rel && currentFile ? getLiteratureImageUrl(folderName, currentFile.name, rel) : rawSrc;
        return <img src={resolved} alt={alt || ""} loading="lazy" className={imageClassName} />;
      },
    };
...
function DesktopLiteratureImage({ folder, filename, image, alt, className }: { folder: string; filename: string; image: string; alt: string; className: string }) {
  ...
  useEffect(() => {
    ...
    let revoke: (() => void) | undefined;
    setUrl("");
    void loadLiteratureImageUrl(folder, filename, image)
      .then(loaded => {
        if (disposed) {
          loaded.revoke?.();
          return;
