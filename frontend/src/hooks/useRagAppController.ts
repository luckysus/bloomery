...
import { getConversationDraft, saveConversationDraft } from "../desktop/services/drafts";
...
const AGENT_NEW_DRAFT_KEY = "__agent_new__";
...
export function useRagAppController() {
  ...
  const [agentDraftReady, setAgentDraftReady] = useState(false);
  useEffect(() => {
    setData(null);
    setQuery("");
  ...
  // 文献处理相关状态
  const literatureUpload = useLiteratureUpload();
  ...
  // 桌面端：输入草稿按会话存入本地 SQLite，切换会话/重启后恢复
  const agentDraftKey = agentSessionId || AGENT_NEW_DRAFT_KEY;

  useEffect(() => {
    if (!isTauriRuntime() || !isAuthenticated || !isAgentMode) {
      setAgentDraftReady(false);
      return;
    }
    let cancelled = false;
    setAgentDraftReady(false);
    void getConversationDraft(agentDraftKey)
      .then((draft) => {
        if (cancelled) return;
        setQuery(draft);
        setAgentDraftReady(true);
      })
      .catch(() => {
        if (!cancelled) setAgentDraftReady(true);
      });
    return () => {
      cancelled = true;
    };
  }, [agentDraftKey, isAgentMode, isAuthenticated]);

  useEffect(() => {
    if (!isTauriRuntime() || !isAuthenticated || !isAgentMode || !agentDraftReady) return;
    const timer = window.setTimeout(() => {
      void saveConversationDraft(agentDraftKey, query).catch(() => undefined);
    }, 250);
    return () => window.clearTimeout(timer);
  }, [agentDraftKey, agentDraftReady, isAgentMode, isAuthenticated, query]);
