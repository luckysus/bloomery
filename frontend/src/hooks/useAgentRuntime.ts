...
import { streamDesktopAsk } from "../desktop/services/localAsk";
import { confirmDesktopCloudJob, streamDesktopAgent } from "../desktop/services/localAgent";
import { saveMemory } from "../desktop/services/memories";
import { isTauriRuntime } from "../desktop/services/tauri";
import { API_BASE, authHeaders } from "../services/api";
import { searchRetrieval } from "../services/search";
...
function readPendingConfirmationArg(item: AgentPendingConfirmation, key: string) {
...
const runAgentRetrievalOptimizationFlow = useCallback(async (
  ...
  try {
    const resp = await searchRetrieval(body);
    ...
    let accumulated = "";
    const updateOptimizationAnswer = () => {
      setAgentMessages(prev => {
        const next = [...prev];
    ...
    try {
      if (isTauriRuntime()) {
        const finalText = await streamDesktopAsk({ query: prompt, contexts, mode: "advice" }, (delta) => {
          accumulated += delta;
          updateOptimizationAnswer();
        }, controller.signal);
  ...
  try {
    if (isTauriRuntime()) {
      if (confirmedActionIds.length > 0) {
        throw new Error("桌面端云任务确认必须通过本地确认命令处理。");
      }
      let streamedAnswer = "";
      ...
      const flushPendingAnswer = () => {
        ...
        setAgentMessages(prev => {
          ...
          if (!hasAgentMessage || nextMessages[nextMessages.length - 1]?.role !== "agent") {
            nextMessages.push({ role: "agent" as const, content: nextAnswer });
      ...
      const appendLocalDelta = (delta: string) => {
        if (!delta) return;
        streamedAnswer += delta;
        pendingAnswerText = streamedAnswer;
        setAgentStreaming(true);
        setAgentProgress(prev => buildAgentProgress("answer", "正在由桌面端本地后端生成回答...", prev.mode));
        if (answerFrame === null) {
          answerFrame = window.requestAnimationFrame(flushPendingAnswer);
        }
      };

      const localResponse = await streamDesktopAgent({
        sessionId: agentSessionId || undefined,
        message: requestBody.message,
      }, appendLocalDelta, controller.signal);
      if (answerFrame !== null) {
        window.cancelAnimationFrame(answerFrame);
        flushPendingAnswer();
      }
      appendFinalResponse(localResponse);
      setAgentProgress(prev => ({ ...prev, active: false, completed: AGENT_PROGRESS_ORDER, current: "answer", statusText: "已由桌面端本地后端完成" }));
      return;
    }
...
const confirmAgentAction = useCallback(async (item: AgentPendingConfirmation, approved: boolean) => {
  ...
  try {
    if (isTauriRuntime()) {
      const conversationId = readPendingConfirmationArg(item, "conversation_id") || agentSessionId;
      const taskType = readPendingConfirmationArg(item, "task_type") || item.tool_name;
      const message = readPendingConfirmationArg(item, "user_message") || "";
      if (!conversationId || !taskType) {
        throw new Error("Cloud task confirmation is missing required desktop metadata.");
      }
      const next = await confirmDesktopCloudJob({
        conversationId,
      ...
      setAgentSessionId(next.session_id || conversationId);
      setAgentMessages(prev => {
        const nextMessages = [...prev, { role: "agent" as const, content: next.answer, response: next }];
        persistAgentConversation(next.session_id || conversationId, nextMessages, next);
        return nextMessages;
      });
      return;
    }
    ...
    if (isTauriRuntime()) {
      if (message?.content?.trim()) {
        await saveMemory({
          scope: "global",
          type: "feedback",
          title: `Agent feedback: ${rating}`,
          description: reason?.trim() || "Saved from desktop feedback.",
          body: message.content.slice(0, 2000),
          tags_json: JSON.stringify(["agent-feedback", rating]),
          enabled: false,
        });
      }
      return;
...
const clearAgentMemory = useCallback(async () => {
  ...
  if (!isTauriRuntime()) {
    await fetch(`${API_BASE}/api/agent/sessions/${agentSessionId}/memory`, {
      method: "DELETE",
      headers: authHeaders(),
    }).catch(() => {});
  }
