...
async function streamAskResponse(
  ...
  payload: { query: string; contexts: string[]; mode?: string },
  ...
  if ("__TAURI_INTERNALS__" in window) {
    let accumulated = "";
    try {
      const finalText = await streamDesktopAsk(payload, (delta) => {
        accumulated += delta;
        setAiAnswer(accumulated);
      }, controller.signal);
      if (finalText && finalText !== accumulated) setAiAnswer(finalText);
    } finally {
...
export function useSearchMode({
  ...
  useEffect(() => {
    ...
    const fetchFilteredProduction = async () => {
      setLoading(true);
      const body = buildQueryBody("", true);
      try {
  ...
  const handleSearch = useCallback(async () => {
    ...
    if ((!query.trim() && !shouldSearchProduction) || loading) return;
    setLoading(true);
    setData(null);
    setAiAnswer("");
    setResultView(isAIMode ? "ai" : "rag");

    const body = buildQueryBody(query.trim(), shouldSearchProduction);
    try {
      const res = await searchRetrieval(body);
      const json = await readSearchResponse(res);
      setData(json);
