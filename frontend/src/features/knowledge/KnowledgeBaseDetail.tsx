...
export default function KnowledgeBaseDetail({
  ...
  const [rawDocumentUrl, setRawDocumentUrl] = useState("");
  const [rawSheets, setRawSheets] = useState<{ name: string; html: string }[]>([]);
  ...
  const markdownComponents = useMemo<Components>(() => {
    const resolveImageSrc = (src?: string) => {
      ...
      return preview?.asset_data_urls?.[rel] || src;
    };
    return {
      img: ({ src, alt }) => {
  ...
  const openRename = () => {
    ...
    setRenameValue(stripDocumentExtension(currentFile.name));
    setRenaming(true);
  };

  const confirmRename = async () => {
    if (!currentFile) return;
    const nextName = ensureDocumentExtension(renameValue.trim(), currentFile.name);
  ...
  const toggleRawDocument = () => {
    if (!currentFile?.document_id) return;
    if (!showRawPdf && !rawDocumentUrl) {
      setRawLoading(true);
      void desktop.getKnowledgeDocumentRaw(currentFile.document_id)
        .then(raw => {
          setRawDocumentUrl(raw.data_url);
          setRawSheets(raw.sheets);
        })
        .catch(() => setRawDocumentUrl(""))
        .finally(() => setRawLoading(false));
    }
    setShowRawPdf(prev => !prev);
  };
        ...
        <section className="min-w-0 min-h-0 flex flex-col bg-[#f7f3ed]">
          <div className="relative flex h-14 shrink-0 items-center justify-between gap-4 border-b border-[#eadccf] bg-[#fffdf8] px-4">
            <div className="flex min-w-0 items-center gap-2 text-base text-[#241b15]">
              ...
              <h3 className="truncate font-medium">{currentFile?.name || folderName}</h3>
              {currentFile && (
                <button
                  type="button"
                  onClick={openRename}
              ...
              <div className="grid h-full max-md:h-auto" style={{ gridTemplateColumns: pdfGridTemplate }}>
                {showRawPdf && (
                  <div className="min-h-0 border-r border-slate-200 bg-white max-md:h-[70dvh] max-md:border-r-0 max-md:border-b">
                    {rawLoading ? (
                      <div className="flex h-full items-center justify-center text-sm text-[#8f8174]">
                        正在加载原始文档...
                      </div>
                    ) : rawDocumentUrl ? (
                      <RawDocumentViewer url={rawDocumentUrl} title={currentFile.name} sheets={rawSheets} />
                    ) : (
...
function ensureDocumentExtension(name: string, originalName: string) {
  ...
  return `${name}${originalName.match(/\.[^.]+$/)?.[0] || ".pdf"}`;
}
