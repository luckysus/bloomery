...
import PostgresWikiPanel from "./PostgresWikiPanel";
import type { LiteratureFileInfo, LiteratureFilePreview } from "./knowledgeTypes";

type MinerUProcessingConfigInfo = {
...
export default function KnowledgeBaseWizard(props: KnowledgeBaseWizardProps) {
  ...
  return (
    <div data-testid="knowledge-web-workspace" className="fixed inset-0 z-50 flex flex-col bg-[#f7f3ed] text-slate-950">
      <div className="flex h-16 shrink-0 items-center gap-3 border-b border-[#e5d8cc] bg-[#fffaf3]/95 px-6 shadow-[0_1px_0_rgba(72,52,38,0.04)] max-md:h-14 max-md:gap-2 max-md:px-3">
        ...
        <button type="button" onClick={() => setWikiOpen(true)} className="inline-flex items-center gap-2 rounded-lg border border-[#e4d6c8] bg-white px-3 py-2 text-sm font-semibold text-[#6f4a38] hover:bg-[#faf0e8]" title="打开 PostgreSQL Wiki">
          <BookOpen size={15} aria-hidden="true" /> Wiki
        </button>
      ...
      {wikiOpen && <PostgresWikiPanel onClose={() => setWikiOpen(false)} />}
            ...
            <section className="min-w-0 min-h-0 flex flex-col rounded-2xl border border-[#e5d8cc] bg-[#fffaf3] p-4 shadow-sm max-md:min-h-[74dvh] max-md:p-2">
              <div className="mb-3 flex shrink-0 items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => {
                    startAppendToCurrentFolder();
                    void props.onChooseFiles?.();
                  }}
                  disabled={props.uploadBusy || props.litLoading !== null}
                  aria-label="点击上传或拖拽文档到这里"
                  className="mr-2 rounded-lg border border-[#e4d6c8] bg-white px-3 py-2 text-sm font-semibold text-[#6f4a38] hover:bg-[#faf0e8] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  上传文档
                </button>
                <button
          ...
          {props.step === 3 && (
            <div className="h-full overflow-auto">
              <div className="mx-auto max-w-[1670px] pb-24">
                <h3 className="mb-5 text-lg font-semibold text-[#241b15]">{props.processingLabel || "服务器处理中"}</h3>
                <div className="space-y-3">
                  {props.jobs.filter(job => !selectedFolderName || job.folder === selectedFolderName).map(job => (
                    <JobRow
                      ...
                      onRetry={() => void props.onRetryJob(job.job_id)}
                    />
                  ))}
      ...
      <div className="relative">
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-4">
          ...
          <div className="flex items-center gap-3 text-sm text-slate-700">
            ...
            {job.status === "failed" && (
              <button onClick={onRetry} className="rounded p-1.5 text-slate-400 hover:bg-slate-100 hover:text-[#b85f43]" title="重试任务" aria-label="重试任务">
                <RefreshCw className="w-4 h-4" />
              </button>
            )}
