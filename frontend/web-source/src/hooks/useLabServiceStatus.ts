...
export function useLabServiceStatus() {
  ...
  const handleLabServiceAction = useCallback(() => {
    void refreshLabServiceStatus({ force: true, reconnect: !labServiceStatus?.optimization_available });
  }, [labServiceStatus?.optimization_available, refreshLabServiceStatus]);
  ...
  const labCardTone: LabCardTone = !labServiceStatus
    ? "slate"
    : labOptimizationAvailable ? "emerald" : "red";
