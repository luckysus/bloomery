...
export default function LoginPage({ onLogin, authClient = "web" }: LoginPageProps) {
  ...
  const [username, setUsername] = useState(() => {
    try {
      return localStorage.getItem("steel_agent_remember_username") || "";
    } catch {
      return "";
    }
  });
  const [rememberAccount, setRememberAccount] = useState(() => {
    try {
      return Boolean(localStorage.getItem("steel_agent_remember_username"));
    } catch {
      return false;
    }
  });
  ...
  const [agreeLegal, setAgreeLegal] = useState(() => {
    try {
      return localStorage.getItem("steel_agent_legal_agreed") === "1";
    } catch {
      return false;
    }
  });
  ...
  const activeProvider: CaptchaProvider = captchaConfig.provider;
    ...
    async function loadPublicSecurityConfig() {
      try {
        const [captchaResp, authResp] = await Promise.all([
          fetch(`${API_BASE}/api/auth/captcha-config`),
          fetch(`${API_BASE}/api/auth/security-config`),
        ]);
        if (captchaResp.ok) {
  ...
  const openSliderChallenge = async (purpose: "email" | "login" | "register") => {
    ...
    try {
      const resp = await fetch(`${API_BASE}/api/auth/slider-captcha`);
      const json = await resp.json().catch(() => ({}));
  ...
  const verifySliderCaptcha = async () => {
    ...
    setSliderError("");
    try {
      const resp = await fetch(`${API_BASE}/api/auth/slider-captcha/verify`, {
        method: "POST",
  ...
  const submitLogin = async (sliderToken?: string) => {
    ...
    try {
      ...
      try {
        if (rememberAccount) localStorage.setItem("steel_agent_remember_username", username.trim());
        else localStorage.removeItem("steel_agent_remember_username");
      } catch {
    ...
    <div className="flex h-screen overflow-hidden bg-[#f5f0e8] text-[#2b2118]">
      ...
      <section className="relative flex min-w-0 flex-1 flex-col overflow-y-auto bg-[#f5f0e8]">
        <header className="flex shrink-0 items-center gap-2.5 px-6 pt-5 lg:px-10">
          <img
            src="/steel-agent-logo.png?v=2"
            alt="钢铁智能体"
            className="h-9 w-9 rounded-xl shadow-[0_4px_12px_rgba(126,66,47,0.3)]"
            draggable={false}
          />
          <span className="text-lg font-bold tracking-wide text-[#2b2118]">钢铁智能体</span>
        </header>
            ...
            <label className="mt-4 flex cursor-pointer items-start gap-2 text-xs leading-relaxed text-[#6f6258]">
              <input
                ...
                onChange={(event) => {
                  ...
                  try {
                    if (checked) {
                      localStorage.setItem("steel_agent_legal_agreed", "1");
                    } else {
                      localStorage.removeItem("steel_agent_legal_agreed");
                    }
                  } catch {
        ...
        <footer className="shrink-0 px-4 pb-4">
          <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-center text-[13px] leading-relaxed text-[#9c8e7f]">
            <span>© 2026 钢铁智能体 · All Rights Reserved</span>
            <a
            ...
            <a
              href="https://beian.mps.gov.cn/#/query/webSearch?code=13012502000102"
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1 transition-colors hover:text-[#cc785c]"
            >
              <img src="/gongan.png" alt="公安备案" className="h-[18px] w-[18px] shrink-0" />
              冀公网安备13012502000102号
            </a>
          </div>
        </footer>
      </section>
